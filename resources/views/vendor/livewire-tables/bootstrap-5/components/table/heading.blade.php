...
@unless ($sortingEnabled && $sortable)
    <th {{ $attributes->class(['text-nowrap'])->merge($customAttributes) }}>
        {{ $text ?? $slot }}
    </th>
@else
    <th
        wire:click="sortBy('{{ $column }}', '{{ $text ?? $column }}')"
        {{ $attributes->class(['text-nowrap', 'cursor-pointer'])->merge($customAttributes) }}
    >
        <div class="d-flex align-items-center">
            <span>{{ $text }}</span>

            <span class="relative d-flex align-items-center ms-auto">
                @if ($direction === 'asc')
